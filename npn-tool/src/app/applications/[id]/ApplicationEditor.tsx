...
const DOC_LABELS: Record<string, string> = {
  pla_form: "PLA Form (.html)", cover_letter: "Cover Letter", fps_form: "Finished Product Specifications",
  label_en: "Label (English)", label_fr: "Label (French)", monograph_attestation: "Monograph Attestation",
  safety_report: "Safety Report", efficacy_report: "Efficacy Report", animal_tissue_form: "Animal Tissue Form",
  senior_attestation: "Senior Attestation", ingredient_specs: "Ingredient Specifications",
  non_med_list: "Non-Med Ingredient List", quality_chemistry_report: "Quality/Chemistry Report",
};
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="w-full lg:w-80 shrink-0 space-y-1.5">
              <button onClick={generateAllDocs} disabled={!!generating}
                className="w-full px-4 py-2.5 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 disabled:opacity-50 mb-3">
                {generating ? `Generating ${generating}...` : "Generate All Documents"}
              </button>
                ...
                <>
                  <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 shrink-0">
                    <span className="text-sm font-semibold text-gray-900">
                      {DOC_LABELS[app.documents.find(d => d.id === activeDocId)?.documentType || ""] || "Document"}
                    </span>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { const doc = app.documents.find(d => d.id === activeDocId); if (doc) generateDoc(doc.documentType); }}

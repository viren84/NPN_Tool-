...
export default function ProductPipelineClient({ user, initialProducts }: Props) {
  ...
  const filtered = products.filter((p) => {
    ...
    if (searchLower && !p.name.toLowerCase().includes(searchLower) && !p.brandName.toLowerCase().includes(searchLower)) return false;
    return true;
  });
        ...
        <div className="mb-4">
          <div className="relative max-w-sm">
            ...
            <input
              ...
              className="w-full pl-10 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent bg-white text-gray-900"
            />
          </div>
        </div>
